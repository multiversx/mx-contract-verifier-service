...
export class DockerRunner {
  ...
  public exec(options: DockerRunnerOptionsInterface): Promise<string> {
    this.logger.log(`Started docker process with options: ${JSON.stringify(options)}`);

    const mountArgs = [];

    if (options.inputVolumeMap) {
      mountArgs.push("--volume", `${options.inputVolumeMap.from}:${options.inputVolumeMap.to}`);
    }

    if (options.outputVolumeMap) {
      mountArgs.push("--volume", `${options.outputVolumeMap.from}:${options.outputVolumeMap.to}`);
    }
